"""Vendored reference: RealMLP-TD-S standalone implementation.

Source: https://github.com/dholzmueller/realmlp-td-s_standalone (MIT,
Copyright (c) 2024- David Holzmueller). Vendored unmodified except for:
the import path, np.NaN/np.Inf -> np.nan/np.inf (NumPy 2 compatibility),
a fitted_ marker for newer scikit-learn's check_is_fitted, and this header. Used only by benchmarks/ as the parity reference.
"""
...
class RobustScaleSmoothClipTransform(sklearn.base.BaseEstimator, sklearn.base.TransformerMixin):
    def fit(self, X, y=None):
        ...
        self.fitted_ = True  # vendct: for newer sklearn check_is_fitted
        return self
