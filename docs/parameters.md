...
| `tower_groups` | `None` | Opt-in **full-depth** disjoint source groups: a list of lists partitioning indices of `embedding.feature_chunk_sizes` exactly once (same contract as `first_layer_groups`, and the two cannot be combined). With two or more groups the model becomes that many parallel towers over the disjoint coordinate blocks, kept separate through *every* hidden layer, and one packed output layer sums their readouts (`logit = sum_g h_g @ W_g / sqrt(width) + b`, one shared bias) — no hidden-layer remixing and no gate. `hidden_sizes` then means the widths of **each** tower, so G towers of `(w, w, w)` cost about `G * w^2` per deep layer, not `w^2`. The first layer of every tower takes `first_layer_lr_factor`, the data-driven init walks each tower on its own slice, and the coordinate index buffers persist through save/load. `None`, or a single group holding every chunk (in any order), is the dense model, byte for byte. Requires at least one hidden layer with a positive width. |
...
entries = per-layer width — and, under `tower_groups`, the widths of *each*
tower rather than of the model as a whole. The `(256, 256, 256)` default is the RealMLP-TD(-S)
