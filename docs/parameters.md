...
| `xla_fuse_steps` | `1` | XLA/TPU only (no effect on other devices): optimizer steps per XLA graph barrier. `1` executes one graph per step; `K > 1` fuses `K` steps into one XLA program, amortizing the per-step dispatch overhead that dominates small-model fits at tabular batch sizes on TPU. Barrier placement does not change results (same seed ⇒ same model, any `K`). |
