...
- **KI-014 — tabm measurement coverage.** Full TabM, quantile PLE-vB, and
  independently shuffled member batches are fidelity/parity tested on CPU
  and covered together by the XLA:CPU static-graph smoke. Binary and
  regression retain their objective/sample-weight contract tests. There is
  still **no at-scale TPU quality or performance verdict**: XLA:CPU confirms
  lowering and static shapes, not TPU throughput or campaign quality.
