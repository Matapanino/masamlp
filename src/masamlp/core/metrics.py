...
Same contract as repleafgbm's metrics module (same author): metrics are
NumPy callables on prediction-scale outputs — probabilities for
classification, original-scale values for regression — so sklearn metrics
can be wrapped directly with :func:`make_metric`.
"""
...
class LogLoss(BaseMetric):
    ...
    def __call__(self, y_true: np.ndarray, y_pred: np.ndarray) -> float:
        # float64 before clipping: in float32 the upper bound rounds to 1.0
        # exactly and saturated predictions would hit log(0).
...
class MultiLogLoss(BaseMetric):
    ...
    def __call__(self, y_true: np.ndarray, y_pred: np.ndarray) -> float:
        ...
        p = y_pred[rows, y_true.astype(np.int64)].astype(np.float64)
...
class BalancedAccuracy(BaseMetric):
    ...
    holds integer class indices (argmax prediction). Matches
    ``sklearn.metrics.balanced_accuracy_score`` (without the chance
    adjustment); classes absent from ``y_true`` are ignored."""
...
def make_metric(
    ...
    Args:
        fn: ``(y_true, y_pred) -> float``. ``y_pred`` is on the prediction
            scale (probabilities for classification, original-scale values
            for regression).
